      ...
      <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
        ...
        <tr><td style="padding:32px 40px;">
          <table width="100%" cellpadding="0" cellspacing="0">
            ...
            ${telefono ? `<tr><td style="padding:12px 0;border-bottom:1px solid #e5e7eb;">
              <strong style="color:#6b7280;font-size:13px;text-transform:uppercase;">Telefono</strong><br>
              <span style="color:#1f2937;font-size:15px;">${escapeHtml(telefono)}</span>
            </td></tr>` : ''}
            ${empresa ? `<tr><td style="padding:12px 0;border-bottom:1px solid #e5e7eb;">
        ...
        <tr><td style="padding:24px 40px;background:#f9fafb;border-top:1px solid #e5e7eb;">
          <p style="margin:0;color:#9ca3af;font-size:13px;text-align:center;">SmartStay Cloud — Solicitud comercial</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
...
async function handleSalesRequest(request, env) {
  ...
  if (!emailRegex.test(email)) {
    return new Response(
      JSON.stringify({ success: false, error: 'Email no valido.' }),
      { status: 400, headers: { 'Content-Type': 'application/json', ...corsHeaders } }
    );
  }

  const RESEND_API_KEY = env.RESEND_API_KEY;
  const RESEND_FROM = env.RESEND_FROM || 'SmartStay <noreply@smartstaycloud.com>';
  ...
  if (!resendRes.ok) {
    ...
    return new Response(
      JSON.stringify({ success: false, error: 'Error al enviar el email.', detail: errText }),
